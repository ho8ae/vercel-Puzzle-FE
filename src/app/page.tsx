...
export default function Home() {
  ...
  const [puzzlePieces, setPuzzlePieces] = useState<{ letter: string; color: string }[]>([]);
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen py-2">
      <div className="w-full max-w-4xl mx-auto px-4">
        ...
        >
          {isClient && puzzlePieces.map((piece, index) => (
            <PuzzlePiece
              key={index}
              letter={piece.letter}
              color={piece.color}
        ...
        >
          다양한 아이디어를 하나로 모아 혁신적인 프로젝트를 만들어보세요. 
          우리의 플랫폼은 당신의 창의성을 극대화합니다.
        </motion.p>
        {!token && (
          <motion.div
